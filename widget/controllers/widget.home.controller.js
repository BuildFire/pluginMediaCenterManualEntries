            ...
            function ($scope, $window, DB, AppDB, COLLECTIONS, $rootScope, Buildfire, Messaging, EVENTS, PATHS, Location, Orders, $location) {
                ...
                Buildfire.auth.getCurrentUser((err, user) => {
                    ...
                    MediaCenter.get().then(function success(result) {
                        ...
                        $rootScope.globalPlaylistLimit = MediaCenterInfo.data.content.globalPlaylistLimit;
                    },
                        function fail() {
                            MediaCenterInfo = _infoData;
                            WidgetHome.media = MediaCenterInfo;
                    ...
                    var onUpdateCallback = function (event) {
                        if (event.tag == "MediaCenter") {
                            if (event.data) {
                                ...
                                $rootScope.globalPlaylistLimit = WidgetHome.media.data.content.globalPlaylistLimit;
                                $scope.$apply();
                    ...
                    $rootScope.addAllToGlobalPlaylist = ($event) => {
                        ...
                        if ($rootScope.areAllInGlobalPlaylist()) {
                            ...
                            GlobalPlaylist.deleteAll(itemsIds).then(() => {
                                itemsIds.forEach((id) => $rootScope.globalPlaylist.playlist = {});
                    ...
                    $rootScope.toggleItemGlobalPlaylist = ($event, item) => {
                        $event.stopImmediatePropagation();
                    ...
                    let delayInterval;
                    let delayTimeOut;
                    $rootScope.playNextItem = (options) => {
                        if (options && options.userInput) return WidgetHome.goToMedia($rootScope.currentIndex + 1);
                    ...
                    WidgetHome.loadMore = function () {
                        ...
                        // For auto play, we need to get the full list of items.
                        if($rootScope.autoPlay) {
